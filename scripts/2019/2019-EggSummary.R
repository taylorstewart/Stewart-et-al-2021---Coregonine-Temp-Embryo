...
hatch.dates <- data %>% 
  filter(rearing == 1) %>%
  group_by(treatment) %>% 
  summarize(mean.dpf = ceiling(mean(dpf)),
            mean.date = as.Date(mean.dpf, origin = "2018-12-01")) %>% ungroup() %>% 
  mutate(act.date = as.Date(c("2019-09-04", "2019-07-16")),
         length = act.date - mean.date)
